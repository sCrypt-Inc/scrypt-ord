...
export class OrdinalNFT extends SmartContract {
    @prop(true)
    isOrdinalNFT: boolean
    ...
    @method()
    buildNFTStateOutput(): ByteString {
        const stateScript = OrdinalNFT.removeInsciption(this.getStateScript())
    ...
    buildMintNFTOutput(
        ...
        const part1 = OrdinalNFT.createInsciption(content, contentType)
        const part2 = OrdinalNFT.removeInsciption(script)
    ...
    static isP2PKHOrdinal(script: ByteString): boolean {
        return (
            len(script) > 25n &&
            OrdinalNFT.isP2PKH(slice(script, 0n, 25n)) &&
            OrdinalNFT.sizeOfOrdinal(slice(script, 25n)) > 0n
        )
    }
    ...
    static removeInsciption(scriptCode: ByteString): ByteString {
        const inscriptLen = OrdinalNFT.sizeOfOrdinal(scriptCode)
    ...
    static sizeOfOrdinal(script: ByteString): bigint {
        ...
        if (
            ...
            pos += 7n
            const contentTypeLen = OrdinalNFT.skipBytes(slice(script, pos))
            if (contentTypeLen > 0n) {
                pos += contentTypeLen
                if (slice(script, pos, pos + 1n) === OpCode.OP_0) {
                    pos += 1n
                    const contentLen = OrdinalNFT.skipBytes(slice(script, pos))
    ...
    async mint(inscription: Inscription) {
        this.setNOPScript(OrdinalNFT.create(inscription))
